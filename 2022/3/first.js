const Run = (inputlist) => {
    ...
    inputlist.forEach(sack => {
        const sackCount = sack.length / 2;
        const sackHalfCount = sack.length / 2;
        const firstCompartment = sack.match(new RegExp('.{0,' + sackHalfCount + '}', 'g'))[0].split("");
        const secondCompartment = sack.match(new RegExp('.{' + sackHalfCount + ',' + sackCount + '}', 'g'))[1].split("");
        // console.warn(sack);
        // console.warn(firstCompartment, secondCompartment);
        ...
        // console.log([... new Set(resultArr)].length);
        if (resultItem == resultItem.toLowerCase()) {
            const points = (alphabet.indexOf(resultItem) + 1);
            total += points;
        } else {
            const points = (26 + alphabet.indexOf(resultItem.toLowerCase()) + 1);
            total += points;
        }
    });
    console.log(total);
